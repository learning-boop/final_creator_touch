...
export default function LogoScene(){
 ...
 useEffect(()=>{
  ...
  Promise.all([new GLTFLoader().loadAsync("/assets/images/logo/creator-touch-3d.glb"),fetch("/assets/images/logo/creator-touch-regions.bin").then(r=>{if(!r.ok)throw new Error("Logo regions unavailable");return r.arrayBuffer();})]).then(([g,regionBuffer])=>{
   ...
   object.traverse(o=>{if(o.isMesh){const old=Array.isArray(o.material)?o.material:[o.material];old.forEach(m=>m.dispose());const count=o.geometry.attributes.position.count;if(regions.length!==count)throw new Error("Logo region count mismatch");const colors=new Float32Array(count*3);const palette=[new THREE.Color("#1F6F8B"),new THREE.Color("#cc0066"),new THREE.Color("#169b19")];for(let i=0;i<count;i++)palette[regions[i]].toArray(colors,i*3);o.geometry.setAttribute("color",new THREE.BufferAttribute(colors,3));o.material=material;}});scene.add(model);container.classList.add("is-loaded");
  }).catch(()=>{});
