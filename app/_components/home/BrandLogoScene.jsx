...
export default function LogoScene(){
 ...
 useEffect(()=>{
  ...
  const material=new THREE.MeshPhysicalMaterial({color:0xffffff,vertexColors:true,metalness:.15,roughness:.45,clearcoat:.3,transparent:true,opacity:1});
  scene.add(new THREE.HemisphereLight(0xffffff,0x333333,1.4));
  const pink=new THREE.DirectionalLight(0xffffff,1.8);pink.position.set(-3,2,4);scene.add(pink);
  const blue=new THREE.DirectionalLight(0xffffff,1.0);blue.position.set(3,-1,2);scene.add(blue);
  ...
  Promise.all([new GLTFLoader().loadAsync("/assets/images/logo/creator-touch-3d.glb"),fetch("/assets/images/logo/creator-touch-regions.bin").then(r=>{if(!r.ok)throw new Error("Logo regions unavailable");return r.arrayBuffer();})]).then(([g,regionBuffer])=>{
   ...
   object.position.sub(center);model=new THREE.Group();model.add(object);baseScale=1.8/Math.max(size.x,size.y,size.z);model.scale.setScalar(baseScale);model.scale.y*=-1;
   object.traverse(o=>{if(o.isMesh){const old=Array.isArray(o.material)?o.material:[o.material];old.forEach(m=>m.dispose());const count=o.geometry.attributes.position.count;if(regions.length!==count)throw new Error("Logo region count mismatch");const colors=new Float32Array(count*3);const palette=[new THREE.Color("#006EBB"),new THREE.Color("#cc0066"),new THREE.Color("#169b19")];for(let i=0;i<count;i++)palette[regions[i]].toArray(colors,i*3);o.geometry.setAttribute("color",new THREE.BufferAttribute(colors,3));o.material=material;}});scene.add(model);container.classList.add("is-loaded");
  }).catch(()=>{});
